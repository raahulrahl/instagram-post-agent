...
def extract_marketing_parameters_with_llm(input_text: str) -> dict:
    ...
    try:
        ...
        extraction_task = Task(
            description=prompt,
            expected_output="Valid JSON string with marketing parameters",
...
async def initialize_crew() -> None:
    """
    Initializes the LLM connection only. 
    Agents and Crews are now instantiated per-request to ensure statelessness.
    """
    global llm_instance
...
async def run_copy_crew(product_website: str, product_details: str, marketing_params: dict) -> str:
    ...
    try:
        ...
        ad_copy = validate_post_content(ad_copy, "ad_copy")
...
async def run_image_crew(product_website: str, product_details: str, ad_copy: str) -> str:
    ...
    try:
        ...
        image_descriptions = validate_post_content(image_descriptions, "image_descriptions")
...
async def handler(messages: list[dict[str, str]]) -> str:
    ...
    # Extract product website and details from input
    ...
    # Extract additional details from the rest of the input
